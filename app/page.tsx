...
export default function FormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitResponse = await fetch(config.submitEndpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData) })
      if (!submitResponse.ok) throw new Error("Failed to submit form")
      toast({ title: "Success!", description: "Form submitted successfully" })

      const redirectResponse = await fetch(config.redirectEndpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ formData, defaultRedirect: config.redirectUrl }) })
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-md mx-auto mb-4 flex justify-end">
          <Select value={language} onValueChange={(value: Language) => setLanguage(value)}>
            ...
            <SelectContent><SelectItem value="en">English</SelectItem><SelectItem value="hi">हिंदी (Hindi)</SelectItem></SelectContent>
          </Select>
        </div>

        <div className="text-center mb-8">
          <Image src="/karya_web.svg" alt="Karya Web Logo" width={120} height={60} className="h-16 w-auto mx-auto mb-4" />
          <h1 className="text-3xl font-bold text-[#41B47D] mb-2">{textContent.pageTitle}</h1>
        ...
        <div className="max-w-md mx-auto">
            ...
            <CardContent className="p-6 bg-[#C8E56E]/10">
              {!isFormDisabled && geolocationStatus !== 'success' && (
                <div className="text-center p-2 mb-4 text-gray-700">
                  ...
                  {geolocationStatus === 'denied' && <p className="text-red-600">Location access is required.</p>}
                </div>
              )}
                  ...
                  {config.fields.map((field) => {
                    ...
                    return (
                      <div key={field.id} className="space-y-2">
                        <Label htmlFor={field.id}>{fieldText.label}{field.required && <span className="text-[#41B47D] ml-1">*</span>}</Label>
                        {field.type === "select" ? (
                          <Select value={formData[field.id] || ""} onValueChange={(value) => handleInputChange(field.id, value)} required={field.required}>
                            <SelectTrigger disabled={isFormDisabled}><SelectValue placeholder={fieldText.placeholder} /></SelectTrigger>
                            <SelectContent>{fieldText.options?.map((option) => (<SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>))}</SelectContent>
                          </Select>
                        ) : (
                          <Input id={field.id} type={field.type} placeholder={fieldText.placeholder} value={formData[field.id] || ""} onChange={(e) => handleInputChange(field.id, e.target.value)} required={field.required} min={field.min} max={field.max} disabled={isFormDisabled} />
                        )}
                      </div>
                    )
                  })}
                </fieldset>
                <Button type="submit" disabled={isSubmitting || isFormDisabled} className="w-full bg-[#41B47D] hover:bg-[#41B47D]/90">
                  {isSubmitting ? (<>{textContent.submittingButtonText}</>) : (textContent.submitButtonText)}
                </Button>
              </form>
            </CardContent>
          </Card>
          {textContent.links.length > 0 && (
            <div className="mt-8 text-center flex flex-wrap justify-center gap-4">
              {textContent.links.map((link, index) => (<a key={index} href={link.url} target="_blank" rel="noopener noreferrer" className="text-[#41B47D] hover:text-[#41B47D]/80 text-sm underline">{link.text}</a>))}
            </div>
          )}
        </div>
      </div>
